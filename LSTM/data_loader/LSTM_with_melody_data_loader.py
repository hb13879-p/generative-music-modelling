...
class LSTMMelodyDataLoader(BaseDataLoader):
    ...
    def __load_data(self, seq_length):
        X = np.genfromtxt(
            os.path.abspath(
                "C:/Users/User/Documents\Project/Dataset/CSV_Outputs/labels.csv"
            ),
            delimiter=",",
        )
        Y = np.genfromtxt(
            os.path.abspath(
                "C:/Users/User/Documents/Project/Dataset/CSV_Outputs/Y_concat.csv"
            ),
            delimiter=",",
        )
        melody = pickle.load(
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(
            X, Y, test_size=0.18, shuffle=False
        )
