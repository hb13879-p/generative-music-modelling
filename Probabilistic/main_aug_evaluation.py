...
def main():
    ...
    model = pickle.load(open(os.path.abspath(r"C:\Users\User\Documents\Project\Models/Probabilistic/evaluation/final_models/hmm_aug_training_set_model.p"),"rb"))
    data_loader = pickle.load(open(os.path.abspath(r"C:\Users\User\Documents\Project\Models/Probabilistic/evaluation/final_data_loaders/hmm_aug_training_set_data_loader.p"),"rb"))
    obs_test = data_loader.get_obs_test_seq()
    state_seq_test = data_loader.get_state_test_seq()
    count = 0
    total = 0
    for obs_list,test_data in zip(list(zip(obs_test[0],obs_test[1],obs_test[2],obs_test[3],obs_test[4],obs_test[5]))[0:-1],list(zip(state_seq_test[0],state_seq_test[1],state_seq_test[2],state_seq_test[3],state_seq_test[4]))[0:-1]):
