from data_loaders.simple_evaluation_data_loader import HmmMelodyDataLoader
from models.simple_evaluation_model import HmmMelodyModel
...
def main():
    ...
    model.train()
    pickle.dump( model, open( os.path.abspath(r"evaluation/final_models/simple_hmm_training_set_model.p"), "wb" ) )
    pickle.dump( data_loader, open( os.path.abspath(r"evaluation/final_data_loaders/simple_hmm_training_set_data_loader.p"), "wb" ) )
    ...
    count = 0
    total = 0
    for obs_list,test_data in zip(list(zip(obs_test[0],obs_test[1])),list(zip(state_seq_test[0],state_seq_test[1],state_seq_test[2],state_seq_test[3],state_seq_test[4]))):
