...
def main():
    ...
    pickle.dump(
        model,
        open(
            os.path.abspath(r"evaluation/final_models/simple_hmm_training_set_model.p"),
            "wb",
        ),
    )
    pickle.dump(
        ...
        open(
            os.path.abspath(
                r"evaluation/final_data_loaders/simple_hmm_training_set_data_loader.p"
            ),
            "wb",
        ),
    )
    ...
    for obs_list, test_data in zip(
        list(zip(obs_test[0], obs_test[1])),
        list(
            zip(
                state_seq_test[0],
                state_seq_test[1],
