...
class MultinomialHMM(object):
    ...
    def __check_init_dims(self,obs_space):
        assert any(isinstance(el, list) for el in obs_space), "Please wrap 1d list in another list"

    def __check_fit_dims(self,obs_seq):
        #check obs_seq 3dims
        assert any(isinstance(el, list) for el in obs_seq[0]),"Please make sure obs_seq is a 3d list of shape (n_obs_seq,n_sequence_samples,sample_seq_len)"
        assert len(obs_seq) == self.n_obs_seq, "n_obs_seq doesn't match number of sequences in obs_seq"
        ...
        assert np.all(find_differences),"Some observation sequences are different lengths across observation types"

    def __check_initialise_params_dims(self,beta):
        #check beta 3dims
        assert beta[0].ndim == 2,"Please make sure beta is a list of 2d emission matrices shape (n_hidden_states,len(obs_space))"
        for i in range(self.n_obs_seq):
            assert len(beta[0][i]) == len(self.obs_space[i]),"Please make sure beta is a 3d list of shape (n_obs_seq,n_hidden_states,len(obs_space))"
    ...
    def __init_beta(self,obs,hidden_states):
        ...
        for i in range(self.n_obs_seq):
            ...
            hidden_state_obs_pairs = [item for sublist in hidden_state_obs_pairs for item in sublist]
    ...
    def __check_vit_dims(self,y):
        assert any(isinstance(el, list) for el in y), "Please wrap 1d list in another list"
    ...
    def __calculate_sequence_likelihood_eq1(self,T):
        ...
        self.PA = np.divide(Ptilde, partition, out=np.zeros_like(Ptilde), where=partition!=0)
