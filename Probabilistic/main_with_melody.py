from data_loaders.hmm_with_melody_data_loader import HmmMelodyDataLoader
...
def main():
    ...
    model.train()
    # model.print_params()
    # come fly: [[1,4,3,8,1,1,6,11],[3,0,4,1,3,1,3,1],[1,1,6,8,10,8,1,1],[3,3,-1,-1,0,10,8,-1],[8,6,-1,-1,11,9,6,-1],[-1,-1,-1,-1,-1,-1,-1,-1]]
    ...
    pickle.dump(
        model,
        open(os.path.abspath(r"evaluation/final_models/hmm_melody_model.p"), "wb"),
    )
    pickle.dump(
        data_loader,
        open(
            os.path.abspath(r"evaluation/final_data_loaders/hmm_melody_data_loader.p"),
            "wb",
        ),
    )
